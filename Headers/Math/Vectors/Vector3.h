...
namespace ZVK
{
	...
	public:
		...
		static float DotProduct(const Vector3f& vec1, const Vector3f& vec2)
		{
		...
		static Vector3f CrossProduct(const Vector3f& vec1, const Vector3f& vec2)
		{
			float cX = (vec1.y * vec2.z) - (vec1.z * vec2.y);
			float cY = (vec1.x * vec2.z) - (vec1.z * vec2.x); 
	...
	inline bool operator==(const Vector3ui& lhs, const Vector3ui& rhs)
	{
		return lhs.x == rhs.x && lhs.y == rhs.y && lhs.z == rhs.z;
	}
