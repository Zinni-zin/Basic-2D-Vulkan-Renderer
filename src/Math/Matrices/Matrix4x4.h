...
#pragma region Constructors
		Matrix4x4() // Initialize with identity matrix
		{
			m_cells[0][0] = 1.0f;
			m_cells[0][1] = 0.0f;
			m_cells[0][2] = 0.0f;
		...
		Matrix4x4(float val, bool isIdentity = true)
		{
			if (isIdentity)
			{
				m_cells[0][0] = val;
...
#pragma region Transformations
		...
		static Matrix4x4 LookAtLH(const Vec3& eye, const Vec3& at, const Vec3& up)
		{
			Vec3 zAxis = Vec3::Normalize(at - eye, false);
			Vec3 xAxis = Vec3::Normalize(Vec3::CrossProduct(up, zAxis));
			Vec3 yAxis = Vec3::CrossProduct(zAxis, xAxis);

			float dotX = Vec3::DotProduct(xAxis, eye);
		...
		{
			if (isRightHand) return LookAtRH(eye, at, up);
		...
		// Transform vec2 with no rotation and scale set to 1 by default
		static Matrix4x4 Transform(const Vec2& translation, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * RotateY(0) * Translate(translation.x, translation.y);
		}

		// Transform vec2 with rotation in the y-axis, scale set to 1 by default
		static Matrix4x4 TransformR(const Vec2& translation, float rotation, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * RotateY(rotation) * Translate(translation.x, translation.y);
		}

		// Transform vec3 with no rotation and scale set to 1 by default 
		static Matrix4x4 Transform(const Vec3& translation, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * Translate(translation.x, translation.y);
		}

		// Transform vec3 with rotation in the x-axis, scale set to 1 by default
		static Matrix4x4 TransformRX(const Vec3& translation, float rotationX, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * RotateX(rotationX) * Translate(translation);
		}

		// Transform vec3 with rotation in the y-axis, scale set to 1 by default
		static Matrix4x4 TransformRY(const Vec3& translation, float rotationY, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * RotateX(rotationY) * Translate(translation);
		}

		// Transform vec3 with rotation in the z-axis, scale set to 1 by default
		static Matrix4x4 TransformRZ(const Vec3& translation, float rotationZ, float scaleFactor = 1.0f)
		{
			return Scale(scaleFactor) * RotateX(rotationZ) * Translate(translation);
		}
#pragma endregion
...
#pragma endregion
		...
		void Print(bool canPrintBorder = false)
		{
			for (int i = 0; i < 4; ++i)
			{
				for (int j = 0; j < 4; ++j)
				{
					if (j == 0 && canPrintBorder) std::cout << "| ";

					std::cout << m_cells[i][j] << " ";

					if (j == 3 && canPrintBorder) std::cout << "|";
				}
